      ...
      <div className="flex w-full justify-between space-x-4 p-4">
        <div className="flex-1 truncate space-y-2">
          <div className="flex items-center gap-4">
            ...
            {item.tags?.length > 0 && (
              <div className="flex items-center gap-1 bg-primary-700 text-white px-2 py-1 rounded-full text-xs font-semibold">
                {item?.tags.map((item, i) => (
                  <span key={i}>{`${item.name} ${item.margin}%`}</span>
                ))}
              </div>
            )}
          </div>
          <div className="flex items-center space-x-3 mt-1">
            ...
            {item.sub_category?.name.length > 0 ? (
              <h3 className="truncate text-sm text-grey-500">
